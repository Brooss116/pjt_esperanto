...
// import { DELETE_NEED } from "../gql/DeleteNeed.tsx";
...
export async function deleteNeed(userId: number, needId: number): Promise<any> {
    try {
        const result = await ApolloClientCall.mutate({
            mutation: DELETE_NEED,
            variables: {
                userId: userId,
                needID: needId,
        ...
        console.log(result.data.deleteNeed);        
    } catch (error) {
        console.error(error);
    }
}
